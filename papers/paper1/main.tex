...
\todo{Phase 4 is built and evaluated. The gate this section set --- does the rule beat
both ranking and doing nothing on realised money --- is \emph{partially} met: it beats
ranking, and it does not beat doing nothing. Section~\ref{sec:sensitivity} shows the
gate was not achievable as written, since the two halves trade off against each other,
and localises a genuine defect in Equation~\ref{eq:abstain} (a constant $\alpha$).
That correction is unevaluated; this marker stays until it is.}
...
ranking consistently, and spends roughly a third as much doing it. Neither beats the
do-nothing baseline.}
\label{tab:abstain}
\end{table}

Table~\ref{tab:abstain} splits the gate. Abstention beats ranking on 65--80\% of draws
while contacting roughly a third as many customers. It does \emph{not} beat doing
nothing at any size tested, and neither does the comparator.
...
Raising the effect size does flip the gate --- at a mean effect of $-0.050$ the rule
beats doing nothing on $57\%$ of draws --- but the flip is not worth having, for three
reasons visible in the same sweep. The two win rates move in \emph{opposite} directions,
and at no setting is either significantly above chance while the other also is: the
regime in which abstention beats inaction is the regime in which blanket treatment beats
abstention, which profits on $88\%$ of draws by the time the gate passes. The share of
