% ---------------------------------------------------------------------------
% RetainIQ -- merged paper 1+2 (see docs/DECISIONS.md D-042 for the positioning).
%
% STATUS: DRAFT. Every section reports experiments that are implemented, tested
% and reproducible from this repository, including Section 8. Every number is
% regenerable; papers/paper1/README.md lists the command for each table.
%
% Section 8 numbers post-date the units correction of Section 8.2. Do not restore
% figures from an earlier revision without re-running the command in the README.
%
...
seeds (Table~\ref{tab:killtest}, drawn in Figure~\ref{fig:killtest}). Ranking by
churn score loses money and loses
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../figures/fig01_kill_test.png}
\caption{The instrument, and the mechanism it exists to reproduce. \textbf{Left:}
expected incremental value against retention budget, averaged over six seeds with
$\pm1$\,s.d.\ bands. Risk-ranked targeting (red) lies below random (grey) at every
budget, and both lie below doing nothing. \textbf{Right:} why. Customers are sorted
into deciles of \emph{predicted} risk and coloured by their \emph{true} type. Harmed
customers are 48\% of the decile a risk-ranked campaign contacts first and 2\% of the
decile it never reaches. The ranking is not uninformative for this decision; it is
anti-informative. Note that the oracle curve peaks and then falls --- even a perfect
rule loses money if forced to spend more.}
\label{fig:killtest}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\textwidth]{../figures/fig03_when_uplift_pays.png}
\caption{Table~\ref{tab:corr} drawn. The horizontal axis is
$\corr(\hat\CATE,\hat\pi)$; the vertical axis is the advantage of effect-based over
outcome-based targeting. Four real settings cluster near zero advantage across a wide
range of positive correlation; the single negative-correlation setting sits an order
of magnitude above them. Deliberately no fitted curve: with five points the ordering
among the positive-correlation settings is within noise, and the figure states this on
its face. The claim is the gap at negative correlation, not a functional form.}
\label{fig:corr}
\end{figure}
...
better; that is false in advertising, and Table~\ref{tab:corr} and
Figure~\ref{fig:corr} show it. It is that
...
(Table~\ref{tab:smalln}). Figure~\ref{fig:smalln} places the two views side by side:
the conventional mean-performance plot and the reliability plot disagree about whether
these methods are usable at the scale in question.
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../figures/fig02_small_n_reliability.png}
\caption{Two views of the same experiment on Hillstrom, with the evaluation set held
fixed so that only estimation difficulty varies. \textbf{Left:} the conventional
report --- mean performance rises with training size, and every method looks
serviceable. \textbf{Right:} the proportion of seeds on which each method actually
beats random. A firm gets one draw, not an average over twenty. Below roughly 2{,}000
customers (shaded) the best method fails one attempt in four, and one estimator is
indistinguishable from a coin flip --- while its mean remains respectable. Reporting
expectation rather than reliability conceals exactly the risk that matters at this
scale.}
\label{fig:smalln}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../figures/fig05_survival_calibration.png}
\caption{Calibration on Telco, which is the property the value calculation needs and
concordance does not measure. A model may rank customers correctly and still be
systematically wrong about absolute survival probability, in which case every
$\CATE_i \cdot V_i$ product built on it is scaled wrongly. Reliability curves for the
discrete-time hazard and the baselines are shown against the diagonal; the
proportional-hazards baselines are visibly optimistic in the region where most of the
value sits.}
\label{fig:survival}
\end{figure}

Kaplan--Meier is in Table~\ref{tab:survival} for a reason, and Figure~\ref{fig:survival}
shows the reliability curves behind it. On calibration error
...
This section reports a partial negative, and we state its shape up front. The rule
beats ranking almost always and does not beat doing nothing.
Section~\ref{sec:sensitivity} shows the gate was not achievable as written, because its
two halves trade off against each other. Section~\ref{sec:units} reports a units error
that invalidated an earlier revision of these numbers and the discipline that failed to
catch it. Section~\ref{sec:ladder} reports what did finally make money, which was not
choosing customers.
...
on what an estimated rule can achieve, not evidence for one.

\subsection{What the estimated rule achieves}

We implement Equation~\eqref{eq:abstain} with a hierarchical Bayesian logistic model
whose heterogeneity scale $\sigma_\gamma$ is marginalised over rather than fixed, and
whose posterior is validated against NUTS (widths agree to within 1\%, means to four
decimals). Ground truth is withheld from every policy. The comparator is deliberately
the strong one: it ranks by $-\hat\CATE_i V_i - c$ using the \emph{same} estimator and
the same customer values, so the comparison isolates abstention rather than the fact
that customers differ in worth.
...
\bottomrule
\end{tabular}
\caption{Realised money, 20 draws per size, ground truth withheld. Abstention beats
ranking almost always, spending a fraction as much; it does not beat doing nothing,
and on roughly three-quarters of draws it \emph{is} doing nothing.}
\label{tab:abstain}
\end{table}

Table~\ref{tab:abstain} splits the gate. Abstention beats ranking on 93\% of draws
overall while contacting a small fraction as many customers (mean realised value
$-524$ against $-2{,}919$ for ranking). It does \emph{not} beat doing nothing, and the
third column explains why the second is so low: on about three-quarters of draws the
rule treats nobody and scores exactly zero. Reporting only ``beats doing nothing''
would score a rule that correctly declines identically to one that loses money, which
is precisely the property under examination, so both are given.

Sweeping $\alpha$ shows the safety property behaving exactly as specified. At
$\alpha = 0.05$ the rule treats \emph{zero} customers and returns the do-nothing
baseline rather than losing money: it correctly identifies that it does not know
enough. There is no $\alpha$ at which it turns a profit --- it either loses money or
declines to act.

This is the result Section~\ref{sec:smalln} predicts. A decision rule inherits the
quality of its inputs, and at these sample sizes the inputs are unreliable. Abstention
makes the \emph{consequences} of that unreliability survivable; it cannot manufacture
reliability that is not in the data. Cross-tenant pooling, with a prior drawn from ten
established firms, cuts mean losses from $-457$ to $-62$ at $n = 500$ by making the
small firm treat two customers instead of twelve --- the same character of result.

The defensible claim is therefore narrower than the one we set out to make: \emph{given
that risk-ranked targeting destroys value (Section~\ref{sec:instrument}), a rule that
reliably returns to zero is worth real money relative to current practice}. That is not
the same as beating the do-nothing baseline, and we do not claim it is.

\subsection{Why the gate could not have been passed}
\label{sec:sensitivity}

A negative result is only informative if one can say what would have changed it, so we
swept the two quantities that govern the answer. Neither sweep changes a default; the
numbers above stand as reported.

Begin with arithmetic that precedes any decision rule. Treating $i$ pays exactly when
$-\CATE_i V_i > c_i$, so the break-even effect is $c_i / V_i$. Under our calibration and
the reference offer that threshold is $0.040$ against a mean effect of $0.010$. An oracle
knowing every $\CATE_i$ would treat $5.8\%$ of customers. That is a ceiling, not an
estimate: the gate asked an estimated rule to locate profit inside $5.8\%$ of a
population from a pilot of a few hundred.

Raising the effect size does flip the gate --- at a mean effect of $-0.077$ the rule
beats doing nothing on $68\%$ of draws --- but the flip is not worth having, for three
reasons visible in the same sweep. The two win rates move in \emph{opposite} directions
and monotonically: as the effect grows, beating ranking falls from $90\%$ to $48\%$
while beating inaction rises from $5\%$ to $68\%$, and at no swept setting is either
significantly above chance while the other also is. The regime in which abstention beats
inaction is the regime in which blanket treatment beats abstention, which profits on
$88\%$ of draws by the time the gate passes. The share of
harmed customers falls from $27\%$ to $3\%$ along the way, so a passing configuration is
one in which the adversarial mechanism of Section~\ref{sec:corr} has been removed. And
the settings that pass most clearly lie outside the band our calibration procedure
targets. \textbf{The gate as we wrote it demanded two properties that trade off against
each other}; that is a defect in the gate, and we do not read it as evidence for the
method.

The second sweep is the more useful one, and it is a criticism of our own design. We
evaluated on a $20\%$ discount costing $32$ --- the second most expensive rung of the
offer ladder. A feature nudge costs $0.10$ and clears its break-even for $69\%$ of
customers. No rung passes the gate, but the cheap and expensive rungs fail for opposite
reasons: discounts produce effects large enough to detect and too small to pay for,
while the nudge is comfortably profitable and its effect ($-0.002$) is too small to
detect at $n \approx 250$. \emph{Detectability and profitability are anti-correlated
across the ladder}, and we regard this squeeze, rather than the effect size, as the
substantive finding of the section.

Finally, one hypothesis is recorded as refuted. The effect-size sweep suggested
abstention was a minimax-regret hedge --- never best, but never catastrophic, with
normalised maximum regret of $34.9\%$ against $100\%$ for both doing nothing and
treating everyone. Because that reading was formed after seeing the failure, we
pre-registered it and tested it on the offer ladder, which played no part in forming it.
It failed: random assignment attains lower maximum regret ($58.9\%$ versus $85.7\%$).
The failure is nonetheless diagnostic. Abstention's worst regret falls on the cheap
rungs, and the per-rung optimum of $\alpha$ moves from $0.49$ on a $0.10$ nudge to
$0.05$ on a $33$ discount --- with a cheap-but-harmful pause offer also wanting $0.05$,
showing the driver is payoff asymmetry rather than cost. \textbf{A constant $\alpha$ is
therefore wrong by construction}: Equation~\ref{eq:abstain} conflates uncertainty with a
reason not to act, when uncertainty about a nearly free intervention is not
decision-relevant. We state the correction --- $\alpha$ as a function of the payoff ratio
--- and deliberately do not evaluate it here, since repairing a flaw in the same pass
that discovers it forfeits the independence that made the diagnosis credible.

\subsection{A units error, and why every test passed}
\label{sec:units}

An earlier revision of Table~\ref{tab:abstain} was produced by a decision rule that
multiplied a log-odds ratio by money. We report it because the failure is general and
the way it survived is more instructive than the bug.

The estimator returns $\CATE_i$ on the \textbf{log-odds} scale, as its definition
requires. Equation~\eqref{eq:abstain} then formed $-\CATE_i V_i - c$, which is money
only if $\CATE_i$ is a difference in \emph{probability}. On one draw the rule valued
treating at $-104.5$ where the truth was $+20.5$, against an offer costing $31.5$.

The distortion is not a constant. Since
$\partial(\Delta p)/\partial\CATE \approx p_0(1-p_0)$, treating log-odds as probability
overstates the benefit by roughly $1/(p_0(1-p_0))$: a factor of four at $p_0 = 0.5$ and
\emph{twenty-five} at $p_0 = 0.05$. The inflation is therefore largest for the customers
least likely to leave --- the Sure Thing quadrant --- which is the precise error this
paper exists to characterise, reproduced inside our own decision rule.

\paragraph{Every test passed, and none of them was wrong.} The rule's own test compared
it against $(-\hat\CATE V - c) > 0$: the same expression, so it agreed. The estimator's
tests used synthetic effects generated on the logit scale, so they were internally
consistent too. \textbf{A units error is consistent with itself, and a suite of
self-consistency checks cannot detect one.} The test that now catches it asserts a value
computed by hand: a log-odds shift of $-0.8$ applied to $p_0 = 0.25$ moves the
probability by $-0.1197$, not by $-0.8$.

The correction converts on the probability scale before touching money, using the
baseline from the same fit, and samples $(\eta_{0i}, \CATE_i)$ jointly because they are
correlated and the logistic transform destroys the closed form. The repaired rule
consumes a posterior already denominated in currency, which makes the original mistake
unrepresentable rather than merely fixed.

\paragraph{What it changed, and what it did not.} Mean realised losses fell from
$-3{,}531$ to $-1{,}070$ at $n=2{,}000$, treating $27$ customers rather than $72$, and
the rule now beats ranking on $93\%$ of draws rather than $75\%$. \textbf{No qualitative
conclusion moved.} The gate still fails, for the reason Section~\ref{sec:sensitivity}
gives: the offer is roughly four times too weak to pay for itself, and
$\corr(\hat\CATE, \CATE)$ is $0.13$ at these sample sizes. Corrections to five
pre-registered predictions were recorded before the fix was run; two failed, including
the prediction that the per-offer dependence of $\alpha$ would disappear. It did not,
which means the defect identified in Section~\ref{sec:sensitivity} survived a challenge
we raised against our own conclusion.

\subsection{Choosing the offer rather than the customer}
\label{sec:ladder}

Every experiment to this point holds the intervention fixed and asks who should receive
it. Section~\ref{sec:sensitivity} shows that question is close to degenerate: when the
offer is nearly free, treat everyone; when it is expensive, treat nobody. We therefore
ask the question the ladder was built for --- \emph{which} intervention for which
customer --- learning a separate effect per rung from a randomised multi-arm pilot. This
is a harder test, not an easier one: the pilot splits $K+1$ ways, leaving roughly $35$
customers per arm at $n = 500$.

\begin{table}[t]
\centering
\begin{tabular}{lrrr}
\toprule
Policy & Mean realised value & Share of oracle & Beaten by optimiser \\
\midrule
Optimiser (per-customer rung) & $655$ / $1{,}039$ / $2{,}252$ & 28\% & --- \\
Best rung from the pilot      & $502$ / $-562$ / $1{,}869$    & 13\% & 58\% [0.42, 0.72] \\
Best single rung, hindsight   & $1{,}997$ / $2{,}554$ / $5{,}892$ & 73\% & 13\% [0.05, 0.27] \\
Oracle over rungs             & $2{,}348$ / $3{,}747$ / $8{,}242$ & 100\% & --- \\
\bottomrule
\end{tabular}
\caption{Offer-ladder allocation at $n = 500 / 1{,}000 / 2{,}000$, 15 draws each.
``Hindsight'' selects the best uniform rung on the evaluation set itself and is not
achievable; it is included as an upper bound on ``pick one good offer''.}
\label{tab:ladder}
\end{table}

This is the first estimated policy in this paper to make money on average, and it
captures roughly twice what the achievable alternative does. It is still not a result we
will claim: $58\%$ on 45 draws is not distinguishable from chance.

The finding we do claim is about the comparator. \textbf{A pilot-chosen best rung agrees
with the true best rung on $13\%$ of draws} --- against $17\%$ for a uniform guess among
six --- and returns \emph{negative} expected money at $n = 1{,}000$. Meanwhile the
hindsight-chosen uniform rung captures $73\%$ of the oracle against the optimiser's
$28\%$. Choosing the right default intervention is worth considerably more than matching
interventions to customers, and it is the choice a small business is least equipped to
make for itself. Risk aversion did not help: the risk-neutral setting dominated at every
size, which is the opposite of what the abstention argument would predict, and we report
it because the parameter was swept rather than chosen.
